...
public class Commands
{
    ...
    private static void HelpCmd(TSPlayer player)
    {
        ...
        {
            player.SendInfoMessage("【自动垃圾桶】指令菜单 [i:3456][C/F2F2C7:插件开发] [C/BFDFEA:by] [c/00FFFF:羽学][i:3459]\n" +
             "/air —— 查看垃圾桶菜单\n" +
             ...
             "/air add 或 del 名字 —— [c/87DF86:添加]|[c/F19092:移出]垃圾桶物品", 193, 223, 186);
        }
    }
    #endregion
    ...
    public static void CheckCmd(CommandArgs args, int num)
    {
        ...
        var flag = false;
        args.Player.SendInfoMessage($"以下垃圾物品数量超过【[c/92C5EC:{num}]】的玩家");
        ...
        {
            args.Player.SendMessage($"没有找到垃圾数量超过[c/92C5EC:{num}]的玩家", 193, 223, 186);
        }
    }
    #endregion
    ...
    public static void AirCmd(CommandArgs args)
    {
        ...
        if (args.Parameters.Count == 0)
        {
            ...
            {
                args.Player.SendSuccessMessage($"您的垃圾桶[c/F2BEC0:监听状态]为：[c/F3F292:{data.Auto}]");
                args.Player.SendSuccessMessage($"只有玩家[c/E4EFBC:移动|攻击]才会执行[c/C086DF:清理]");
            }
            return;
        }
        if (args.Parameters.Count == 1)
        {
            ...
            if (args.Parameters[0].ToLower() == "on")
            {
                ...
                args.Player.SendSuccessMessage(data.Enabled ?
                    $"玩家 [{args.Player.Name}] 已[c/92C5EC:启用]自动垃圾桶功能。" :
                    $"玩家 [{args.Player.Name}] 已[c/92C5EC:禁用]自动垃圾桶功能。");
            ...
            if (args.Parameters[0].ToLower() == "auto")
            {
                ...
                args.Player.SendSuccessMessage(data.Auto ?
                    $"玩家 [{args.Player.Name}] 的垃圾桶位格监听功能已[c/92C5EC:启用]" :
                    $"玩家 [{args.Player.Name}] 的垃圾桶位格监听功能已[c/92C5EC:禁用]");
                AutoAirItem.DB.UpdateData(data); // 更新数据库
                return;
            }
            ...
            if (args.Parameters[0].ToLower() == "mess")
            {
                ...
                args.Player.SendSuccessMessage(data.Mess ?
                    $"玩家 [{args.Player.Name}] 的自动清理消息已[c/92C5EC:启用]" :
                    $"玩家 [{args.Player.Name}] 的自动清理消息已[c/92C5EC:禁用]");
                AutoAirItem.DB.UpdateData(data); // 更新数据库
                return;
            }
        }
                ...
                case "remove":
                    {
                        ...
                        args.Player.SendSuccessMessage("已从垃圾桶移出:[c/DEFF7D:{1}]个[c/92C5EC:{0}]", item.Name, del.Value);
