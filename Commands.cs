                    ...
                    case "add":
                        {
                            ...
                            AutoAirItem.Config.Write();
                            args.Player.SendSuccessMessage("已成功将物品添加到垃圾桶: [c/92C5EC:{0}]!\n" + $"您的垃圾桶启用状态为：[c/92C5EC:{config.Enabled}]", item.Name);
                            break;
                        }
                    case "delete":
                    case "del":
                    case "remove":
                        {
                            ...
                            AutoAirItem.Config.Write();
                            args.Player.SendSuccessMessage("已成功从垃圾桶删除物品: [c/92C5EC:{0}]!\n" + $"您的垃圾桶启用状态为：[c/92C5EC:{config.Enabled}]", item.Name);
                            break;
                        }
